...
for q in range(0,7):
	...
	for i in rng:
		...
		print("size of sgd balanced training set:", len(tempx_train))
		print("size of sgd balanced val set:", len(tempx_val))
		...
		clf_sgd.fit(tempx_train, tempy_train)
...
np.savetxt("sgd_VariableTrainSet.csv",overall,delimiter=",")
np.savetxt("sgd_Variance.csv",variance,delimiter=",")
np.savetxt("sgd_AccuracyChange.csv",acc_change,delimiter=",")
