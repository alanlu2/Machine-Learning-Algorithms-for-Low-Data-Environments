'''
THIS STUDY LOOKS AT USING A VARIABLE TRAIN DATASET TO OPTIMIZE CLASSIFICATION ACCURACY ON FASHION_MNIST.  UNBALANCED, SIMPLE MODEL
'''
...
for choose in rng:
    ...
    for iteration in range(total_iterations):
        ...
        for i in range(3):
            ...
            sim_holdscore = siamese_net.evaluate(hold_sim_images, hold_sim_labels,verbose=1)[1]
        ...
        trainscore = snn.evaluate(train_images, train_labels,verbose=1)[1]
        valscore = snn.evaluate(val_images, val_labels,verbose=1)[1]
        holdscore = snn.evaluate(hold_images, hold_labels,verbose=1)[1]
        ...
        orig_tr_images=orig_tr_images.reshape(choose*10,784)
        orig_hold_images=orig_hold_images.reshape(choose*10,784)
        ...
        for k in range(1, 5):
            ...
            bestkmodel.fit(orig_tr_images, orig_tr_labels)

            score = bestkmodel.score(orig_val_images.reshape(choose*10,784), orig_val_labels)
    ...
    np.savetxt("snn_network_accuracies"+str(choose)+".csv",overall_siamese[counter],delimiter=",")
    np.savetxt("snn_testing_accuracies"+str(choose)+".csv",overall_testing[counter],delimiter=",")
    counter=counter+1
print("#########################################")
...
np.savetxt("snn_network_accuracies.csv",overall_siamese,delimiter=",")
np.savetxt("snn_testing_accuracies.csv",overall_testing,delimiter=",")
